...
// ── 상수 ──────────────────────────────────────────────────────
// message 필드 제거 → 이벤트 생성 시 동적으로 주입
...
function StaffMarker({ staff, isSelected, onSelect }: {
  ...
  const arrowDir = ttipAbove ? "M-5,-20 L5,-20 L0,-14 Z" : "M-5,20 L5,20 L0,14 Z";
  ...
  return (
    <g transform={`translate(${x} ${y})`} style={{ cursor: "pointer" }}
      ...
      <text x="0" y="18" textAnchor="middle" fontSize="7" fill={color} fontWeight="700" fontFamily="system-ui,sans-serif">직원{staff.id}</text>
      ...
      {isSelected && (
        <g>
          ...
          <rect x="-62" y={ttipY} width="124" height="28" rx="5" fill="#1f2937" opacity="0.93" />
...
function TerminalFloorMap({
  ...
  return (
    <svg viewBox="0 0 640 358" className="w-full" style={{ display:"block", maxHeight:358 }} aria-label="T1 터미널 플로어맵">
      ...
      {staffList.map(staff => (
        <StaffMarker key={staff.id} staff={staff}
...
export default function AICarePanel({ onCalmRoom }: { onCalmRoom?: () => void }) {
  ...
  const gateIdxRef = useRef(0);           // 관찰중 순환 인덱스
  ...
  // ── 피드 이벤트 추가 헬퍼 ──────────────────────────────────
  const addFeedEventRef = useRef<(type: EventType, gate: string, msg: string) => void>(() => {});
  const addFeedEvent = (type: EventType, gate: string, msg: string) => {
    idRef.current += 1;
    const evt: FeedEvent = { id: idRef.current, time: nowStr(), gate, type, message: msg };
    setFeed(prev => [evt, ...prev.slice(0, 14)]);
  };
  addFeedEventRef.current = addFeedEvent;

  // ── 직원 파견: 가장 가까운 대기중 직원 → 반환값 staffId ──────
  ...
  // ── 인시던트 시퀀스 ───────────────────────────────────────
  // 관찰중 → 불안감지 → 앱푸시발송 → 직원파견(직원N) → 케어중 → 케어완료
  ...
  const triggerIncident = (gateId: string) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        const sid = dp(gateId);
        if (sid === null) return;
        ae("직원파견", gateId, `직원${sid} ${gateId}로 이동 중`);
  ...
  useEffect(() => {
    ...
    const initial: FeedEvent[] = ALL_GATE_IDS.map((id, i) => ({
      ...
      gate: id,
      type: "관찰중",
      message: "AI 비전 분석 모니터링 중",
    }));
    setFeed(initial);
    idRef.current = gates.length;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // ── 관찰중 ticker: 4초마다 게이트 순환 ────────────────────
  useEffect(() => {
  ...
  // ── 인시던트 트리거: 초기 6초 후 1회 + 이후 35초 주기 ────
  useEffect(() => {
    const first = setTimeout(() => {
      triggerRef.current(pick(ANXIETY_GATES));
    }, 6000);
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">게이트·직원 클릭 시 상세 정보 · AI 피드 연동</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5">
              {staffList.map(s => (
                <span key={s.id} className="w-2.5 h-2.5 rounded-full transition-colors duration-500"
          ...
          <div className="space-y-1.5 max-h-72 overflow-y-auto">
            {feed.map(evt => (
              <div key={evt.id} className="flex items-start gap-2 py-2 border-b border-gray-50 last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-xs font-semibold text-gray-700 mr-1">{evt.gate}</span>
                  <span className="text-xs text-gray-400">{evt.message}</span>
                </div>
              </div>
            ))}
          </div>
